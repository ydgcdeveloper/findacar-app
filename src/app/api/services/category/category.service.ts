...
import { CategoryRepoService } from '../../repos/category/category-repo.service';
...
export class CategoryService {
  ...
  async getCategories(): Promise<Category[]> {
    try {
      const categoryResponse = await this.categoryRepo.getCategories();
      if (!categoryResponse.errors) {
        const categoriesData = categoryResponse.data.getCategories;
