...
export const getAvailability = (service): boolean => {
    ...
    const initialTime = set(new Date(), {
      hours: Number((Time as string).split('-')[0].split(':')[0]),
      minutes: Number((Time as string).split('-')[0].split(':')[1]),
    });
    const finalTime = set(new Date(), {
      hours: Number((Time as string).split('-')[1].split(':')[0]),
      minutes: Number((Time as string).split('-')[1].split(':')[1]),
    });
